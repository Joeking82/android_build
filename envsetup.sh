...
function print_lunch_menu()
{
    ...
    if [ "z${TOXYC_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    TOXYC_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/toxyc/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the TOXYC model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch toxyc_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the TOXYC GitHub
        T=$(gettop)
        cd $T > /dev/null
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $TOXYC_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.toxyc.device | grep -q "$TOXYC_BUILD");
    then
        ...
        echo "The connected device does not appear to be $TOXYC_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.toxyc.device | grep -q "$TOXYC_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
